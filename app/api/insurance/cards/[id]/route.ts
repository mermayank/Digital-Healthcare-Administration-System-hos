...
export async function GET(
  ...
  try {
    ...
    return NextResponse.json({ error: 'Failed to fetch insurance card' }, { status: 500 })
  }
}
...
export async function PUT(
  ...
  try {
    ...
    if (!session || (session.user.role !== 'ADMIN' && session.user.role !== 'PATIENT')) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }
    ...
    if (session.user.role === 'ADMIN') {
      ...
      const updatedCard = await prisma.insuranceCard.update({
        ...
        data: {
          ...
          ...(status === 'APPROVED' && { verifiedById: session.user.id, verifiedAt: new Date() })
        },
        include: {
          patient: {
            include: {
    ...
    return NextResponse.json({ error: 'Failed to update insurance card' }, { status: 500 })
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json({ message: 'Insurance card deleted successfully' })
  } catch (error) {
    console.error('Error deleting insurance card:', error)
    return NextResponse.json({ error: 'Failed to delete insurance card' }, { status: 500 })
  }
}